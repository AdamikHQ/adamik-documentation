...
sequenceDiagram
    ...
    APP->>ADAMIK API: /api/transaction/encode
    ADAMIK API->>APP: encoded transaction (pre-validation, fees, etc.)

    APP->>User: request signature
    User->>APP: sign transaction

    APP->>ADAMIK API: /api/transaction/broadcast
    ADAMIK API->>APP: transaction hash
```
